...
def _salestotal_dict(test_cls, testcase):
    ...
    for i, row in enumerate(testcase):
        if row['VIPINFO'] is None:
            if i == len(testcase) - 1:
                test_cls.skipTest('TestCase 中的VIPINFO 缺少 customerCode, vipGradeCenter, vipBonusCenter 的相关信息')
        ...
        # check TestCase 中 VIPINFO 是否缺漏信息
            ...
            if i == len(testcase) - 1:
                test_cls.skipTest('请查看TestCase 中的VIPINFO 的 customerCode, vipGradeCenter, vipBonusCenter 的相关信息是否补全，'
