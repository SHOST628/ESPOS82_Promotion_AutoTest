...
def mk_test_name(name, value, index=0, name_fmt=TestNameFormat.DEFAULT):
    ...
    if name_fmt is TestNameFormat.INDEX_ONLY or not is_trivial(value):
        # TODO start return new testcase name
        # if type(value[1]) is dict:
        #     try:
        #         apiname = value[1]['APINAME']
        #         caseid = value[1]['TESTCASEID']
        #         return "test_{0}_{1}".format(apiname, caseid)
        ...
        if type(value) is tuple:
            try:
                caseid = value[0]
                return "test_{0}_{1}".format(index, caseid)
            except Exception as e:
                raise e
        # TODO end
